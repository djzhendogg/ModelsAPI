...
# New class for inference pipeline
class TesterPipeline(object):
    ...
    def test(self, dataset, max_length, protein_dim, last_epoch):
        """
        Тестирует модель на предоставленном датасете.

        Аргументы:
            dataset (list): Список кортежей (embedding_a, embedding_b, label).
            max_length (int): Максимальная длина последовательности.
            protein_dim (int): Размерность эмбеддинга белка.
            last_epoch (bool): Указывает на последнюю эпоху тестирования.

        Возвращает:
            loss (float): Ошибка на датасете.
            T (list): Истинные метки.
            Y (list): Предсказанные метки.
            S (list): Вероятности предсказаний.
        """
        self.model.eval()
        ...
        with torch.no_grad():
            ...
            loss, correct_labels, adjusted_score = self.model(data_pack, last_epoch=last_epoch, train=False)
            ...
            return loss.item() * len(dataset), T.tolist(), Y.tolist(), S.tolist()